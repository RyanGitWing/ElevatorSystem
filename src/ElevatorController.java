...
public class ElevatorController implements Runnable {
	...
	public void receiveControl(boolean setTimeOut) {
	      ...
	      try {     
	         ...
	         sendReceiveSocket.receive(receivePacket);
	    	  ...
	    	  System.out.println("Scheduler has detected that Elevator " + port + " has exceeded the expected travel time. The elevator has been disabled.\n");
	    	  sendControl((byte) 20);
	    	  working = false;
	    	  return;
	      } catch (IOException e) {
	...
	public void decodeControl(byte[] msg) {
		...
		case 6: //Elevator doors have opened
			if (msg[1] == 1) {
				try {
					System.out.println("Door stuck, now repairing elevator door");
					Thread.sleep(2000);
					sendControl((byte) 0);
					receiveControl(false);
